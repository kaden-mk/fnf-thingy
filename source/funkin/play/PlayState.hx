...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  function initHealthBar():Void
  {
    ...
    scoreText = new FlxText(healthBarBG.x + healthBarBG.width - 600, healthBarBG.y + 30, 0, '', 20);
    scoreText.setFormat(Paths.font('vcr.ttf'), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
    scoreText.scrollFactor.set();
    scoreText.zIndex = 802;

    if (DisableUi.score) add(scoreText);
  ...
  function updateScoreText():Void
  {
    ...
    {
      var tempText:String = "N/A";
      if (accuracy == 0.00) tempText = "N/A";
      else
        tempText = HelperFunctions.truncateFloat(accuracy, 2) + "%";

      scoreText.text = 'Score:' + songScore + ' | Misses: ' + Highscore.tallies.missed + ' | Notes Hit: ' + Highscore.tallies.totalNotesHit
        + ' | Accuracy: ' + tempText;
    }
  }
  ...
  {
    var sick:Int = Highscore.tallies.sick;
    var good:Int = Highscore.tallies.good;
    var bad:Int = Highscore.tallies.bad;
    var shit:Int = Highscore.tallies.shit;
    ...
    totalNotesHit += sick;
    totalNotesHit += good * 0.75;
    totalNotesHit += bad * 0.5;
    totalNotesHit += shit * 0.25;
  ...
  function processNotes(elapsed:Float):Void
  {
    ...
    for (holdNote in opponentStrumline.holdNotes.members)
    {
      ...
      {
        var dad = Preferences.playAsOpponent ? currentStage.getBoyfriend() : currentStage.getDad();

        // Make sure the opponent keeps singing while the note is held.
        if (currentStage != null && dad != null && dad.isSinging())
        {
          dad.holdTimer = 0;
        }
      }
  ...
  function processInputQueue():Void
  {
    ...
    while (inputPressQueue.length > 0)
    {
      ...
      if (!Constants.GHOST_TAPPING && notesInDirection.length == 0 && !Preferences.ghostTapping)
      {
